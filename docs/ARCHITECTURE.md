...
负责服务状态、进程监管、NapCat 端口配置、模块导航、管理员能力令牌、移动端首次配对 API 和云中继出站轮询。高复杂度业务仍由原模块完成，工作站只承担编排和边界控制。
...
桌面端仅在用户主动启用后监听局域网端口，长期访问则通过受限云中继：

1. App 先调用无敏感信息的 `/api/ping`；
2. 六位一次性配对码验证成功后，桌面端注册随机设备账户；服务端只保存令牌哈希，手机只获得一次明文令牌；
3. 配对尝试按来源 IP 限速；
4. 令牌保存在手机系统安全存储中；
5. 配对后可从任意网络向固定中继提交白名单请求，工作站只通过出站轮询执行，不开放本机端口；
...
- **首次配对采用 LAN，长期控制采用白名单中继**：本机端口不暴露公网，同时允许已登记设备跨网络工作；工作站离线时请求不会绕过本地业务边界执行。
- **高频群查询保持本地**：CSV 在启动阶段导入 SQLite，QQ群的每次 `!` 查询只访问本机数据库；云端用于低频运营数据、中继队列和静态资源，避免按查询产生费用。
