...
void main() {
  ...
  test('portfolio demo refresh reads only its isolated library status', () async {
    final requests = <http.Request>[];
    ...
    stopwatch.stop();
    expect((controller.status['songs'] as Map)['total'], 1272);
    expect((controller.status['stable'] as Map)['total'], 142);
  ...
  testWidgets('portfolio demo dashboard hides production service controls', (
    tester,
  ) async {
    final controller = AppController(_EmptySessionStore())
      ..api = WorkstationApi(
    ...
    expect(find.text('隔离体验环境'), findsOneWidget);
    expect(find.text('SongBot'), findsNothing);
    expect(find.text('NapCat'), findsNothing);
    expect(find.text('每日歌曲与竞猜'), findsNothing);
  });
