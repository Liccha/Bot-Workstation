...
> 本仓库是经过脱敏的**只读作品集快照**，用于展示架构与工程实践，不是正式客户端分发渠道。公开副本编译后只显示作品集提示，不启动电脑端、SongBot、MczMaker 或手机运营流程；Web 只保留公开曲库查询。它不连接生产控制面、不使用正式更新源，云端写入默认拒绝。真实管理员凭据、群号、数据库、公告附件、歌曲音频、受许可限制的模板和签名密钥均不在仓库中。
...
`web/fc/` 仅保留架构实现供代码审阅；作品集策略会拒绝所有内容写请求。请勿把本仓库作为正式服务或客户端部署。
...
首次配对仅接受私有局域网地址，桌面管理端口不会暴露到公网。配对完成后，App 保存一个不可读取云密钥、可由桌面立即撤销的设备令牌；此后只访问固定云数据接口，切换 Wi-Fi/移动网络或关闭电脑均不会使账号失效。Android 签名文件不属于源码，也不会进入 Git。App 更新包在应用内下载并完成大小与 SHA-256 校验，再交由 Android 系统确认安装；歌曲、Stable 与静态内容更新不需要重装 App。

## 运行数据与素材

公开仓库不附带生产数据，也不会连接生产控制面。源码可用于审阅和测试，但不是可操作正式数据的内部版本。架构中的完整系统依赖以下私有运行材料，这些内容不会发布：
...
- 这是生产项目的脱敏、只读代码快照，不是生产环境备份或正式客户端。
- GitHub 公开仓库必然允许访客下载源码 ZIP；技术上无法阻止对下载副本的本地修改，但未经修改的作品集副本不能注册生产编辑身份、写入生产数据或更新成正式客户端。
